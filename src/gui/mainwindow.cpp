 ...
 void MainWindow::on_btnGenerate_clicked()
{
    ...
    qDebug() << "UI数据采集完成，准备传入算法核.."
             << "当前DOF:" << m_flightParams.getConfig().dof
             << "当前VelX:" << m_flightParams.getLimits().velX;
    // 在这里调用你的轨迹生成器
    // m_trajectoryGenerator.generate(m_flightParams);
    if (m_simManager) {
        m_simManager->generateTrajectory(m_flightParams);
    } else {
        QMessageBox::critical(this, "System Error", "Simulation manager is not initialized.");
    }
...
void MainWindow::showZoomedTableDialog()
{
    ...
    // 打印按钮
    QAction* actPrint = toolbar->addAction("🖨打印");
    actPrint->setToolTip("打印表格数据");
    connect(actPrint, &QAction::triggered, zoomDialog, []() {
        // 可以在这里实现打印功
        QMessageBox::information(nullptr, "打印", "打印功能暂未实现");
    });

    // 查找按钮
    QAction* actSearch = toolbar->addAction("🔍 查找");
    actSearch->setToolTip("在表格中查找");
    actSearch->setShortcut(QKeySequence::Find);

    toolbar->addSeparator();

    // 刷新按钮
    QAction* actRefresh = toolbar->addAction("🔄 刷新");
    actRefresh->setToolTip("刷新表格数据");
