...
int DataTableWidget::findRowByTime(double time) const
{
    ...
    // Clamp both indices to valid range (defense in depth)
    const int maxIdx = static_cast<int>(m_trajectoryData->points.size()) - 1;
    left = qBound(0, left, maxIdx);
    right = qBound(0, right, maxIdx);
